...
const app = {
  ...
  checkAuth() {
    ...
    try {
      ...
      // Normalize role names from API and seed data to the module names used by the router.
      const rawRole = (this.user.role_name || this.user.role || 'student')
        .toString()
        .trim()
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '_')
      ...
      const roleAliases = {
        ...
        pc: 'program_coordinator',
        programcoordinator: 'program_coordinator'
      };
      this.user.role = roleAliases[rawRole] || rawRole;
  ...
  renderSidebarMenu() {
    ...
    switch(role) {
      ...
      case 'hod':
        items = [
          ...
          { name: 'Faculty Wise Analysis', icon: 'bi-people', path: 'faculty' },
      ...
      case 'program_coordinator':
        items = [
          ...
          { id: 'nav-students', name: 'Students & Upload', icon: 'bi-people', path: 'students' },
          { name: 'Feedback Summary', icon: 'bi-chat-left-dots', path: 'feedback' }
        ];
        break;
      case 'dean':
    ...
    items.forEach(item => {
      ...
      li.innerHTML = `<a${itemId} href="#${item.path}" class="nav-link" data-path="${item.path}"><i class="bi ${item.icon}"></i> ${item.name}</a>`;
      menu.appendChild(li);
    });
