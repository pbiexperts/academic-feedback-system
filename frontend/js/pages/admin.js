...
,
  async renderStudents(container) {
    this.appContext.showLoading();
    try {
      const [students, departments, divisions] = await Promise.all([
      ...
      container.innerHTML = `
        ...
        <div class="card shadow-sm border-0 mb-4">
          <div class="card-header bg-white border-0 pt-3">
            <h5 class="mb-3">Student Directory</h5>
            <div class="row g-3">
              <div class="col-md-6">
                <input type="text" class="form-control" id="studentSearchInput" placeholder="Search by name, enrollment no, email, or phone...">
              </div>
              <div class="col-md-3">
                <select class="form-select" id="studentDeptFilter">
                  <option value="">All Departments</option>
                  ${departments.map(d => `<option value="${d.id}">${d.name}</option>`).join('')}
                </select>
              </div>
              <div class="col-md-3">
                <select class="form-select" id="studentDivFilter">
            ...
            <div class="table-responsive">
              <table class="table table-custom table-hover">
                ...
                <tbody id="studentTableBody">
                  ${students.map(s => `
                    <tr data-dept="${s.department_id}" data-div="${s.division_id}" data-search="${(s.name || '') + (s.enrollment_no || '') + (s.email || '') + (s.phone_no || '')}">
                      <td><strong>${s.enrollment_no}</strong></td>
                      <td>${s.name || 'N/A'}</td>
                      <td>${s.email || 'N/A'}</td>
                      <td>${s.phone_no || 'N/A'}</td>
                      <td><span class="badge bg-primary">${s.department_name || 'N/A'}</span></td>
                      <td>${s.division_name || 'N/A'}</td>
                      <td>
                        <button class="btn btn-sm btn-outline-primary" onclick="adminModule.viewStudentDetails(${s.id})">
      ...
      document.getElementById('studentSearchInput').addEventListener('input', (e) => this.filterStudents());
      document.getElementById('studentDeptFilter').addEventListener('change', (e) => this.filterStudents());
      document.getElementById('studentDivFilter').addEventListener('change', (e) => this.filterStudents());
    } catch (err) {
      this.appContext.showError('mainContent', err.message);
    }
  },
  ...
  filterStudents() {
    ...
    document.querySelectorAll('#studentTableBody tr').forEach(row => {
      ...
      row.style.display = (matchesSearch && matchesDept && matchesDiv) ? '' : 'none';
    });
  },
    ...
    try {
      ...
      document.getElementById('globalModalBody').innerHTML = `
        <dl class="row mb-0">
          ...
          <dt class="col-sm-4">Department</dt><dd class="col-sm-8">${student.department_name || 'N/A'}</dd>
