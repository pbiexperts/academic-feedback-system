...
window.studentModule = {
  ...
  init(app) {
    this.appContext = app;
    this.loadProfileBadge();
  },

  async loadProfileBadge() {
    try {
      const profile = await api.get('/student/me');
      const userName = document.getElementById('userName');
      const userRole = document.getElementById('userRole');
      const profileBadge = document.getElementById('studentProfileBadge');
      if (userName) userName.textContent = profile.name || profile.enrollment_no || profile.email;
      if (userRole) {
        userRole.textContent = 'Student';
        userRole.title = `Enrollment: ${profile.enrollment_no || 'N/A'} | Phone: ${profile.phone_no || 'N/A'}`;
      }
      if (profileBadge) {
        profileBadge.textContent = `${profile.enrollment_no || 'No enrollment'} | ${profile.phone_no || 'No phone'}`;
      }
    } catch (err) {
      // Keep the authenticated email already shown in the header if profile loading fails.
    }
  },
    ...
    try {
      ...
      container.innerHTML = `
        <div class="d-flex justify-content-between align-items-center mb-4">
          <div>
            <h2 class="mb-1">Welcome, ${profile.name || profile.email}</h2>
              <p class="text-muted mb-0">${profile.department_name || 'Department'} &middot; ${profile.enrollment_no}</p>
              <small class="text-muted">${profile.email || 'N/A'} &middot; Registered phone: ${profile.phone_no || 'N/A'}</small>
          </div>
        </div>
        ...
        <tr>
          ...
          <td>
            ${s.is_submitted 
              ...
              : s.is_eligible
                ? `<button class="btn btn-sm btn-primary" onclick="studentModule.startFeedback(${s.subject_id}, ${s.faculty_id}, ${cycleStatus.cycle_id}, '${(s.faculty_name || 'Faculty').replace(/'/g, "\\'")}', '${(s.faculty_email || '').replace(/'/g, "\\'")}')"><i class="bi bi-pencil me-1"></i>Give Feedback</button>`
  ...
  startFeedback(subjectId, facultyId, cycleId, facultyName = '', facultyEmail = '') {
    this.feedbackContext = { subjectId, facultyId, cycleId, facultyName, facultyEmail };
  ...
  async renderFeedbackForm(container) {
    ...
    try {
      ...
      // Get faculty details from context or subject info
      const facultyName = ctx.facultyName || (subjectInfo && subjectInfo.faculty_name) || 'Faculty';
      const facultyEmail = ctx.facultyEmail || (subjectInfo && subjectInfo.faculty_email) || '';
      ...
      container.innerHTML = `
        <div class="d-flex justify-content-between align-items-start mb-4">
          ...
          <div class="d-flex gap-2 align-items-start">
            <div class="card shadow border-0" style="width: 200px; background: linear-gradient(135deg, #f5f7fa 0%, #ffffff 100%);">
              <div class="card-body text-center p-3">
                <div class="mb-3">
                  <i class="bi bi-person-circle" style="font-size: 4rem; color: #1a237e;"></i>
                </div>
                <h6 class="mb-2" style="font-size: 1rem; color: #1a237e; font-weight: 700;">${facultyName}</h6>
                ${facultyEmail ? `<small class="text-muted d-block" style="font-size: 0.8rem; word-break: break-word;">${facultyEmail}</small>` : ''}
                <div class="mt-2 pt-2 border-top">
                  <small class="text-muted">${subjectInfo ? subjectInfo.subject_name : 'Course'}</small>
                </div>
              </div>
            </div>
